...
def _grad_div(clip_gradient, grad):
    mul = P.Mul()
    ret = mul(grad, clip_gradient)
    return ret
...
class TrainOneStepCellWithGradClip(Cell):
    ...
    def __init__(self, network, optimizer, clip_gradient, sens=1.0):
        ...
        self.norm = nn.Norm(keep_dims=False)
        self.dtype = P.DType()
        self.cast = P.Cast()
        self.stack = P.Stack()
        ...
        self.div = P.RealDiv()
        self.add = P.Add()
        self.clip_gradient = Tensor(np.array([clip_gradient]).astype(np.float32))
    ...
    def construct(self, data, label):
        ...
        norms = self.hyper_map(F.partial(compute_norm), grads)
        norms = self.stack(norms)
        norm = self.norm(norms)
        clip_coef = self.div(self.clip_gradient, self.add(norm, 1e-6))
        if clip_coef < 1:
            grads = self.hyper_map(F.partial(grad_div, clip_coef), grads)
        if self.reducer_flag:
